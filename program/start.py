...
""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("➕ Beni Grupa Ekle ➕", url=f"https://t.me/{ASOmusicbot}?startgroup=true")
                ],[
                    InlineKeyboardButton("❓ Asisstant", callback_data="user_guide")
                ],[
                    InlineKeyboardButton("📚 Menyu", callback_data="command_list"),
                    InlineKeyboardButton("❤️ Reklam", url=f"https://t.me/{ismiyev95}")
                ],[
                    InlineKeyboardButton("👥 𓆩𓄂𝙰𝚂𝙾🇦🇿𝐒ö𝐡𝐛𝐞𝐭 𝐐𝐫𝐮𝐩𝐮𝐦𝐮𝐳", url=f"https://t.me/{WerabliAnlar}"),
                    InlineKeyboardButton("📣 𓆩𓄂𝙰𝚂𝙾🇦🇿𝐔𝐬𝐞𝐫 𝐁𝐨𝐭", url=f"https://t.me/{WerabSupport}")
                ],[
...
async def alive(c: Client, message: Message):
    ...
    buttons = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("✨ 𓆩𓄂𝙰𝚂𝙾🇦🇿𝐒ö𝐡𝐛𝐞𝐭 𝐐𝐫𝐮𝐩𝐮𝐦𝐮𝐳", url=f"https://t.me/{GROUP_SUPPORT}"),
                InlineKeyboardButton(
                    "📣 𓆩𓄂𝙰𝚂𝙾🇦🇿𝐔𝐬𝐞𝐫 𝐁𝐨𝐭", url=f"https://t.me/{UPDATES_CHANNEL}"
                ),
            ]
        ]
    )
    text = f"**Hello {message.from_user.mention()}, I'm {me_bot.first_name}**\n\n🧑🏼‍💻 My Master: [{𓆩𓄂𝙰𝚂𝙾🇦🇿❉͜͡᭙Ҩ➣ࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩࠩ𝕋𝕚𝔾𝕣}](https://t.me/{ismiyev95})\n👾 Bot Version: `v{__version__}`\n🔥 Pyrogram Version: `{pyrover}`\n🐍 Python Version: `{__python_version__}`\n✨ PyTgCalls Version: `{pytover.__version__}`\n🆙 Uptime Status: `{uptime}`\n\n❤ **Thanks for Adding me here, for playing video & music on your Group's video chat**"
    await c.send_photo(
        chat_id,
        photo=f"{ALIVE_IMG}",
        caption=text,
...
@Client.on_message(command(["ping", f"ping@{ASOmusicbot}"]) & ~filters.edited)
@check_blacklist()
